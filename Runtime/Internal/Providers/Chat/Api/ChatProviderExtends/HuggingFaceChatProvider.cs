...
{
    internal class HuggingFaceChatProvider : ChatProviderAbstract
    {
        internal override ChatProviderType ProviderType => ChatProviderType.HUGGING_FACE;

        internal HuggingFaceChatProvider(ChatProviderSettings settings_) : base(settings_, HuggingFaceChatApiConfig.BASE_URL)
        {
        }
        ...
        internal override async Task<ChatResponse> SendMessageAsync(ChatRequestPayload requestPayload_, string model_, CancellationToken cancellationToken_ = default)
        {
            try
            {
                ...
                string url = $"{_settings.BaseUrl}{HuggingFaceChatApiConfig.CHAT_ENDPOINT}";
                Dictionary<string, string> headers = HuggingFaceChatApiConfig.GetAuthHeaders(
                ...
                if (!response.IsSuccess)
                {
                    ...
                    LogRequestError(model_ ?? _settings.DefaultModel ?? HuggingFaceChatApiConfig.DEFAULT_CHAT_MODEL, url, body, errorMessage);
            ...
            {
                LogRequestError(model_ ?? _settings.DefaultModel ?? HuggingFaceChatApiConfig.DEFAULT_CHAT_MODEL, null, null, ex.Message);
        ...
        private void LogRequestError(string model_, string url_, Dictionary<string, object> body_, string errorMessage_)
        {
            ...
            AIProviderLogger.LogError($"[HuggingFace Chat] Request failed - Model: {model_}, URL: {url_ ?? "(unknown)"}, Body: {bodyJson}, Error: {errorMessage_}");
        }
            ...
            CancellationToken cancellationToken_ = default)
        {
            ...
            {
                Debug.LogError("HuggingFaceChatProvider: onChunkReceived_ callback cannot be null for streaming.");
                return;
            }
        ...
        private Dictionary<string, object> BuildRequestBody(ChatRequestPayload requestPayload_, string model_, bool isStream_)
        {
            ...
            {
                ["model"] = model_ ?? _settings.DefaultModel ?? HuggingFaceChatApiConfig.DEFAULT_CHAT_MODEL,
            ...
            HuggingFaceChatRequestOptions options = requestPayload_.HuggingFaceOptions;
            if (options != null)
            {
