...
{
    internal class OpenAIChatProvider : ChatProviderAbstract
    {
        internal override ChatProviderType ProviderType => ChatProviderType.OPEN_AI;

        internal OpenAIChatProvider(ChatProviderSettings settings_) : base(settings_, OpenAIChatApiConfig.BASE_URL)
        {
        }
        ...
        internal override async Task<ChatResponse> SendMessageAsync(ChatRequestPayload requestPayload_, string model_, CancellationToken cancellationToken_ = default)
        {
            try
            {
                ...
                string url = $"{_settings.BaseUrl}{OpenAIChatApiConfig.CHAT_ENDPOINT}";
                Dictionary<string, string> headers = OpenAIChatApiConfig.GetAuthHeaders(
                ...
                if (!response.IsSuccess)
                {
                    ...
                    LogRequestError(model_ ?? _settings.DefaultModel ?? OpenAIChatApiConfig.DEFAULT_CHAT_MODEL, url, body, errorMessage);
            ...
            {
                LogRequestError(model_ ?? _settings.DefaultModel ?? OpenAIChatApiConfig.DEFAULT_CHAT_MODEL, null, null, ex.Message);
        ...
        private void LogRequestError(string model_, string url_, Dictionary<string, object> body_, string errorMessage_)
        {
            ...
            AIProviderLogger.LogError($"[OpenAI Chat] Request failed - Model: {model_}, URL: {url_ ?? "(unknown)"}, Body: {bodyJson}, Error: {errorMessage_}");
        }
        ...
        private object SanitizeValue(object value_)
        {
            ...
            if (value_ is Dictionary<string, object> dict)
            {
                if (dict.ContainsKey("data") || dict.ContainsKey("inlineData") || dict.ContainsKey("b64_json"))
                    return "(base64 data)";
                if (dict.ContainsKey("image_url"))
                {
                    Dictionary<string, object> sanitizedDict = new Dictionary<string, object>(dict);
                    if (sanitizedDict["image_url"] is Dictionary<string, object> imageUrl && imageUrl.ContainsKey("url"))
                    {
                        string url = imageUrl["url"]?.ToString();
                        if (url != null && url.StartsWith("data:"))
                            sanitizedDict["image_url"] = "(base64 image data)";
                    }
            ...
            CancellationToken cancellationToken_ = default)
        {
            ...
            {
                Debug.LogError("OpenAIChatProvider: onChunkReceived_ callback cannot be null for streaming.");
                return;
            }
        ...
        private Dictionary<string, object> BuildRequestBody(ChatRequestPayload requestPayload_, string model_, bool isStream_)
        {
            ...
            foreach (ChatRequestMessage msg in requestPayload_.Messages)
            {
                ...
                if (msg.OpenAIOptions != null && !string.IsNullOrEmpty(msg.OpenAIOptions.Name))
                {
                    message["name"] = msg.OpenAIOptions.Name;
                }
            ...
            {
                ["model"] = model_ ?? _settings.DefaultModel ?? OpenAIChatApiConfig.DEFAULT_CHAT_MODEL,
            ...
            OpenAIChatRequestOptions options = requestPayload_.OpenAIOptions;
            if (options != null)
            {
        ...
        private ChatResponse ParseResponse(Dictionary<string, object> json_)
        {
            ...
            ChatResponse response = new ChatResponse
            {
                ...
                RawResponse = json_,
                OpenAIData = new OpenAIChatResponseData
                {
                    SystemFingerprint = JsonHelper.GetValue<string>(json_, "system_fingerprint"),
                    ServiceTier = JsonHelper.GetValue<string>(json_, "service_tier")
                }
            };
