...
namespace Weppy.AIProvider
{
    public partial class GeminiCliWrapper : IDisposable
    {
        ...
        internal GeminiCliWrapper(int timeoutSeconds_ = 120)
        {
            _timeoutSeconds = timeoutSeconds_;
        }

        internal async Task<GeminiCliResponseResult> ExecuteAsync(
            ...
            CancellationToken cancellationToken_ = default)
        {
            ...
            {
                return new GeminiCliResponseResult
                {
                    IsSuccess = false,
                    ExitCode = -1,
                    ErrorMessage = "Gemini CLI executable path is empty."
                };
            }
            ...
            if (string.IsNullOrEmpty(resolvedPath))
            {
                return new GeminiCliResponseResult
                {
                    IsSuccess = false,
                    ExitCode = -1,
                    ErrorMessage = $"Gemini CLI executable '{executablePath_}' not found in PATH or specified location."
                };
            }
                ...
                using (Process process = new Process())
                {
                    ...
                    if (!process.Start())
                    {
                        return new GeminiCliResponseResult
                        {
                            IsSuccess = false,
                            ExitCode = -1,
                            ErrorMessage = "Failed to start Gemini CLI process."
                        };
                    }
                    ...
                    if (finishedTask != exitTask)
                    {
                        ...
                        string errorMessage = cancellationToken_.IsCancellationRequested
                            ? "Gemini CLI request was cancelled."
                    ...
                    GeminiCliResponseResult result = new GeminiCliResponseResult
                    {
        ...
        internal async Task<GeminiCliResponseResult> ExecutePersistentAsync(
            string executablePath_,
            string arguments_,
            string standardInput_,
            Dictionary<string, string> environmentVariables_ = null,
            ...
            CancellationToken cancellationToken_ = default)
        {
            ...
            try
            {
                ...
                {
                    FileName = resolvedPath,
                    Arguments = effectiveArgs ?? string.Empty,
                    RedirectStandardInput = true,
                    RedirectStandardOutput = true,
                    RedirectStandardError = true,
                    UseShellExecute = false,
                ...
                using (Process process = new Process())
                {
                    ...
                    if (string.IsNullOrEmpty(standardOutput))
                    {
                        return new GeminiCliResponseResult
                        {
                            IsSuccess = false,
                            ExitCode = -1,
                            ErrorMessage = string.IsNullOrEmpty(standardError)
                                ? "Gemini CLI response was empty."
                                : standardError,
                            StandardError = standardError
                        };
                    }
            ...
            catch (OperationCanceledException)
            {
                return new GeminiCliResponseResult
                {
                    IsSuccess = false,
                    ExitCode = -1,
                    ErrorMessage = "Gemini CLI request was cancelled."
                };
            }
            catch (Exception ex)
            {
        ...
        private async Task<GeminiCliResponseResult> ReadPersistentOutputAsync(CancellationToken cancellationToken_)
        {
            if (_persistentProcess == null || _persistentOutput == null)
            {
                return new GeminiCliResponseResult
                {
                    IsSuccess = false,
                    ExitCode = -1,
                    ErrorMessage = "Persistent Gemini CLI process is not available."
                };
            }
            ...
            while (true)
            {
                if (_persistentProcess.HasExited)
                {
                    ...
                    return new GeminiCliResponseResult
                    {
                        IsSuccess = false,
                        ExitCode = -1,
                        ErrorMessage = "Gemini CLI process exited unexpectedly."
                    };
                }
                ...
                if (line == null)
                {
                    ...
                    return new GeminiCliResponseResult
                    {
                        IsSuccess = false,
                        ExitCode = -1,
                        ErrorMessage = "Gemini CLI process ended without response."
                    };
                }
        ...
        private void EnsurePathEnvironmentVariable(ProcessStartInfo startInfo_)
        {
            ...
            if (!string.IsNullOrEmpty(homeDir))
            {
                ...
                additionalPaths.Add($"{homeDir}/.npm-global/bin");
                additionalPaths.Add($"{homeDir}/.nvm/versions/node/latest/bin");
            }
#elif UNITY_EDITOR_LINUX || UNITY_STANDALONE_LINUX
            additionalPaths.Add("/usr/local/bin");
            additionalPaths.Add("/usr/bin");
            additionalPaths.Add("/snap/bin");
            string homeDir = Environment.GetEnvironmentVariable("HOME");
            if (!string.IsNullOrEmpty(homeDir))
            {
                additionalPaths.Add($"{homeDir}/.local/bin");
            ...
            {
                additionalPaths.Add(System.IO.Path.Combine(appData, "npm"));
            }
#endif
        ...
        internal static string FindGeminiExecutablePathInternal()
        {
            string[] commonPaths = GetCommonGeminiPathsInternal();
        ...
        internal static string[] GetCommonGeminiPathsInternal()
        {
            ...
            {
                candidates.Add("/opt/homebrew/bin/gemini");
                candidates.Add("/usr/local/bin/gemini");
                candidates.Add("/usr/bin/gemini");
                string homeDir = Environment.GetEnvironmentVariable("HOME");
                if (!string.IsNullOrEmpty(homeDir))
                {
                    candidates.Add($"{homeDir}/.local/bin/gemini");
                    candidates.Add($"{homeDir}/.npm-global/bin/gemini");
                    candidates.Add($"{homeDir}/.nvm/versions/node/latest/bin/gemini");
                }
            }
                ...
                || Application.platform == RuntimePlatform.LinuxPlayer)
            {
                candidates.Add("/usr/local/bin/gemini");
                candidates.Add("/usr/bin/gemini");
                candidates.Add("/snap/bin/gemini");
                string homeDir = Environment.GetEnvironmentVariable("HOME");
                if (!string.IsNullOrEmpty(homeDir))
                {
                    candidates.Add($"{homeDir}/.local/bin/gemini");
                ...
                || Application.platform == RuntimePlatform.WindowsPlayer)
            {
                ...
                if (!string.IsNullOrEmpty(appData))
                {
                    candidates.Add(Path.Combine(appData, "npm", "gemini.cmd"));
                    candidates.Add(Path.Combine(appData, "npm", "gemini.ps1"));
                }
            }
